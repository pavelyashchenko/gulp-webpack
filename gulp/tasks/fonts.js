...
export const ttfToWoff = () => {
	...
	return app.gulp.src(`${app.path.sourceFolder}/fonts/*.ttf`, {})
		...
		.pipe(app.gulp.src(`${app.path.sourceFolder}/fonts/*.{woff,woff2}`))
		// Выгружаем в папку с результатом
		.pipe(app.gulp.dest(`${app.path.build.fonts}`));
}
export const fontsStyle = () => {
	// Файл стилей подключения шрифтов
	let fontsFile = `${app.path.sourceFolder}/styles/fonts.styl`;
	...
	fs.readdir(app.path.build.fonts, function (err, fontsFiles) {
		if (fontsFiles) {
			...
			if (!fs.existsSync(fontsFile)) {
				...
				console.log("Файл styles/fonts.styl уже существует. Для обновления файла нужно его удалить!");
			}
		}
	});
